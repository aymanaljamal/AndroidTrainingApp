package com.example.echoandlistapp.feature.chat;
...
import com.example.echoandlistapp.data.model.Message;
import com.example.echoandlistapp.data.network.Api;
import com.example.echoandlistapp.data.network.ApiClient;
import com.example.echoandlistapp.data.network.ApiResponse;
...
public class ChatActivity2 extends AppCompatActivity {
    ...
    private long myUserId = 1L; // المستخدم الحالي
            ...
            api.sendMessage(chatId, myUserId, text).enqueue(new Callback<ApiResponse>() {
                @Override public void onResponse(Call<ApiResponse> call, Response<ApiResponse> res) {
                    if (res.isSuccessful() && res.body()!=null && "success".equalsIgnoreCase(res.body().status)) {
                        ...
                        Toast.makeText(ChatActivity2.this, err, Toast.LENGTH_SHORT).show();
                    }
                }
                @Override public void onFailure(Call<ApiResponse> call, Throwable t) {
                    Toast.makeText(ChatActivity2.this, "Network error: " + t.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
        });
    }
    ...
    private void loadMessages() {
        api.getMessages(chatId).enqueue(new Callback<List<Message>>() {
            ...
            @Override public void onFailure(Call<List<Message>> call, Throwable t) {
                Toast.makeText(ChatActivity2.this, "Failed to load: " + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
}
